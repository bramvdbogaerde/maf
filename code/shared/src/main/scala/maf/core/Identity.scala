...
/**
 * An identity that is used for instrumentation in order to create unique identities for
 * addresses generated by the instrumentation
 *
 * @param idn the identity of the expression the instrumented expression is based on
 */
case class InstrumentedIdentity(idn: IDN, n: Integer) extends Identity with SmartHash {
  override def toString(): String = s"${idn}:${n}"
}
